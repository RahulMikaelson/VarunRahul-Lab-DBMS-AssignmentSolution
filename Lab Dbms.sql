...
create table Customer(CUS__ID int auto_increment primary key,CUS_NAME varchar(250),CUS_PHONE bigint,CUS_CITY varchar(250),CUS_GENDER varchar(1));
...
create table Orderss(ORD_ID int primary key ,ORD_AMOUNT int, DATE date,CUS__ID int ,PROD_ID int,foreign key(CUS__ID)references Customer(CUS__ID),foreign key(PROD_ID)references ProductDetails(PROD_ID));
...
insert into Orderss(ORD_ID,ORD_AMOUNT,DATE,CUS__ID,PROD_ID) values(20, 1500, '2021-10-12', 3, 5);
insert into Orderss(ORD_ID,ORD_AMOUNT,DATE,CUS__ID,PROD_ID) values(25 ,30500 ,'2021-09-16', 5 ,2);
insert into Orderss(ORD_ID,ORD_AMOUNT,DATE,CUS__ID,PROD_ID) values(26, 2000, '2021-10-05', 1 ,1);
insert into Orderss(ORD_ID,ORD_AMOUNT,DATE,CUS__ID,PROD_ID) values(30, 3500, '2021-08-16', 4 ,3);
insert into Orderss(ORD_ID,ORD_AMOUNT,DATE,CUS__ID,PROD_ID) values(50, 2000, '2021-10-06', 2, 1);
...
select P.CUS_GENDER, count(P.CUS_GENDER) from (select c.CUS__ID, c.CUS_GENDER, o.ORD_AMOUNT from Customer c inner join Orderss o 
...
(select PROD_ID from orderss where CUS__ID = 2));
...
(select PRO_ID from productdetails where PROD_ID in 
(select PROD_ID from orderss where ORD_AMOUNT in 
(select min(ORD_AMOUNT) from orderss))));

-- 7) Display the Id and Name of the Product ordered after “2021-10-05”.
select Q.PRO_ID, p.PRO_NAME, Q.DATE from product p inner join 
(select o.DATE, o.PROD_ID, pd.PRO_ID from orderss o inner join productdetails pd on o.PROD_ID = pd.PROD_ID) as Q 
...
select sum(o.ORD_AMOUNT)as total_males_order_amt from customer c inner join orderss o on o.CUS__ID = c.CUS__ID
where c.CUS_GENDER = 'M'
group by c.CUS_GENDER;

-- 11)  Display all the Customers left outer join with the orders.
select c.CUS_NAME, c.CUS_GENDER, c.CUS_CITY, c.CUS_PHONE from customer c left outer join orderss o on c.CUS__ID = o.CUS__ID;
...
else 'Suppllier Not To be Considered'
